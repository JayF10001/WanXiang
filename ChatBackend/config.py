...
# 加载环境变量文件
# 优先加载特定环境的配置文件，然后加载默认的.env文件
env_files = ['.env.local', '.env.docker', '.env.production', '.env']
...
class Config:
    ...
    HOTNEWS_DAILYHOT_BASE_URL = os.getenv('HOTNEWS_DAILYHOT_BASE_URL', 'https://api-hot.imsyy.top')
    HOTNEWS_IKUNPAY_BASE_URL = os.getenv('HOTNEWS_IKUNPAY_BASE_URL', 'https://api.ikunpay.com')
    ...
    SYSTEM_PROMPT_FILE = os.getenv('SYSTEM_PROMPT_FILE', 'prompts/系统提示词.md')
    SKILL_PROMPT_FILE = os.getenv('SKILL_PROMPT_FILE', 'prompts/skill.md')
    ...
    QWEN_FALLBACK_MODEL = os.getenv('QWEN_FALLBACK_MODEL', 'qwen-max')
    ...
    # Active provider selection: 'qwen', 'openrouter', 'deepseek'
    # If set, use that provider directly; otherwise use default QWEN -> OpenRouter chain
    ACTIVE_PROVIDER = os.getenv('ACTIVE_PROVIDER', '').lower()
