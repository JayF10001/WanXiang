...
class FrontendApiClient:
    ...
    def _login(self) -> None:
        if not settings.frontend_api_bot_email or not settings.frontend_api_bot_password:
            raise RuntimeError("缺少 QQ 机器人服务账号配置：QQ_FRONTEND_API_BOT_EMAIL / QQ_FRONTEND_API_BOT_PASSWORD")
        ...
        if response.status_code >= 400 or not payload.get("success"):
            raise RuntimeError(payload.get("detail") or payload.get("message") or "QQ 机器人服务账号登录 frontend_api 失败")
